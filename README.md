...
**contentType**!). Если при запуске утилиты задан параметр **-append** со списком URI, содержимое указанных URI будет включено в выходной архив в качестве новых разделов:
...
> java -jar da.infer.jar <стандартные аргументы> -ontology ONTOLOGY \[-customRules RULES]

Утилита подерживает весь набор стандартных аргументов. Параметр **-ontology ONTOLOGY** задает онтологию, по правилам которой нужно провести инференсинг. Допустимые значения параметра см. 
нумерацию *chav1961.da.util.interfaces.OntologyType* в данном проекте. 

Если при запуске утилиты задан параметр **-customRules RILES**, то, помимо инференсинга, утилита выполнит над
содержимым раздела дополнительные манипуляции. Параметр **RULES** - любой URI, откуда можно взять список правил манипуляции. Правила манипуляции представляют собой текстовый файл, содержащий один или несколько операторов подмножества [SPARQL](https://ru.wikipedia.org/wiki/SPARQL). Допустимыми операторами являются только CONSTRUCT, UPDATE и DELETE. Один оператор в тексте отделяется от другого строкой "---". Их применение позволяет выполнить дополнительную обработку содержимого раздела (типичный пример - удалить часть выведенных "капитанских" триплов).
...
**после** всех вызовов метода processPart()). Подробности использования данного класса см Javadoc. Желательно также придерживаться соглашения по кодам завершения, описанным в предыдущем разделе.
