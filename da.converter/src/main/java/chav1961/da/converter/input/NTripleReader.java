...
public class NTripleReader implements InputConverterInterface {
	static final String						LITERAL_CACHING = "object.literal.length.caching";
	static final char[]						DUMMY_STRING = "".toCharArray();
	static final SubstitutableProperties	props; 
	...
	final int		literalCaching = props.getProperty(LITERAL_CACHING,int.class);
	final long[]	tempLong = new long[6];
	final int[]		tempInt = new int[2];
	...
	protected int parseSubject(final int lineNo, final char[] data, final int start, int from, final SyntaxTreeInterface<char[]> tree) throws SyntaxException, IllegalArgumentException, NullPointerException {
		switch (data[from]) {
			case '_' :	return parseAnon(lineNo, data, from, tree, ContentWriter.SUBJ_INDEX);
			case '<' :	return parseURI(lineNo, data, from, tree, ContentWriter.SUBJ_INDEX);
			case '#' : case '\r' : case '\n' : return -1;
			default : throw new SyntaxException(lineNo, from-start, "Illegal char ('_', '<', '#' are available only)"); 
		}
	}
	...
	protected int parsePredicate(final int lineNo, final char[] data, final int start, int from, final SyntaxTreeInterface<char[]> tree) throws SyntaxException, IllegalArgumentException, NullPointerException {
		switch (data[from]) {
			case '_' : return parseAnon(lineNo, data, from, tree, ContentWriter.PRED_INDEX);
			case '<' : return parseURI(lineNo, data, from, tree, ContentWriter.PRED_INDEX);
			case '#' : case '\r' : case '\n' :
				throw new SyntaxException(lineNo, from-start, "Missing predicate"); 
			default :
				throw new SyntaxException(lineNo, from-start, "Illegal char ('_', '<', are available only)"); 
...
//	int parseObject(final int lineNo, final char[] data, final int start, int from, final SyntaxTreeInterface<char[]> tree) throws SyntaxException, IllegalArgumentException, NullPointerException {
//		switch (data[from]) {
//			case '_' :
//				from = parseAnon(lineNo, data, from, tree, ContentWriter.OBJ_INDEX);
//				break;
...
//	}
	...
	int parseURI(final int lineNo, final char[] data, int from, final SyntaxTreeInterface<char[]> tree, final int tempLongIndex) throws SyntaxException, IllegalArgumentException, NullPointerException {
		final int 	begin = from + 1;
	...
	int parseAnon(final int lineNo, final char[] data, int from, final SyntaxTreeInterface<char[]> tree, final int tempLongIndex) throws SyntaxException, IllegalArgumentException, NullPointerException {
	...
	long insertIntoTree(final char[] data, final int from, final int to, final SyntaxTreeInterface<char[]> tree, final boolean checkURI) throws SyntaxException, IllegalArgumentException, NullPointerException {
